...
//				users.setUserName(rs.getString(EmployeesTb.email));
	...
	/*public class UsersEmpMapper extends UsersMapper implements RowMapper<Users> {
		public Users mapRow(ResultSet rs, int rowNum) throws SQLException {
			Users users =super.mapRow(rs, rowNum);// new Users();
...
//			super.mapRow(rs, rowNum);
	...
	/*public int numRecords(String searchKey,int role){
		...
		String cont = " where "+UsersTb.user_role_id+" = "+role+" AND "+likeQuery;		
		String SQL = "select count(*) from "+UsersTb.tableName+" as u Join "+EmployeesTb.tableName+" as e "+" on e."+EmployeesTb.id+" = u."+UsersTb.username+cont;
		int numr=jdbcTemplateObject.queryForObject(SQL, Integer.class);
		System.out.println("numr="+numr);
		return numr;
	...
	/*public List<Users> getAllUsers(int start,int showRecsPerPage,int role){
		...
		System.out.println("SQL="+SQL);
		List<Users> usersList = jdbcTemplateObject.query(SQL,new UsersEmpMapper()); 
	...
	/*users*/
	/*public boolean AddEmpUser(String userId,int empId,int userRole){
	...
	/*public int getUserId(int userid1) {
		String SQL = "select * from "+UsersTb.tableName+" where "+UsersTb.username+" = ?";
		Users user = jdbcTemplateObject.queryForObject(SQL,new Object[]{userid1}, new UsersMapper(){});

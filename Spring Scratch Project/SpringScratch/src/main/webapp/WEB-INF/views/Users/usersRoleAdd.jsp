...
<%
	System.out.println("usersRoleAdd ");
	String[] modules = null;
	...
	if (modules != null) {
		for (int i = 0; i < modules.length; i++) {
			System.out.println("id=" + i + ",Val=" + modules[i]);
		}
	}

	String divName = "content-wrapper";
	String baseUrl = request.getContextPath() + "/users";
	String actUrl = request.getContextPath() + "/users/rolePost/act?";
	String postUrl = request.getContextPath() + "/users/rolePost?";
				...
				<div class="col-lg-8 " style="border:1px solid #000">
					<div class="row cusTab">
						<div class="checkbox col-lg-4 " >
							<label><input type="checkbox" name="<%=modules[i] %>" value="0">Read</label>
						</div>
						<div class="checkbox col-lg-4 "  >
							<label><input type="checkbox" name="<%=modules[i] %>" value="1">Write</label>
						</div>
						<div class="checkbox col-lg-4 "  >
							<label><input type="checkbox" name="<%=modules[i] %>" value="2" >Delete</label>
						</div>
					</div>
...
$(document).ready(function() {
	...
	$('#add-form').validate({ // initialize the plugin
        ...
        submitHandler: function(form) {
           ...
           submitFormAllValues(form,'<%=postUrl + "cmd=" + CMD.CREATE%>'+modRoleStr);

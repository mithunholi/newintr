...
window.onpopstate = function(e) {
	if (location.pathname == "/Bumper_Glass/dashboard"
			|| location.pathname == "/Bumper_Glass/deviceOwner_dashboard"
			|| location.pathname == "/Bumper_Glass/advertiser_dashboard") {
		// $.load(location.pathname);
		location.reload();
	} else {
 ...
 * location.pathname="/Bumper_Glass/dashboard"; location.reload(); //alert(
 * "reload?." ); });
 */
...
function menuInit(mid) {
	...
	$("li a", $(mid)).on(
			...
			function(e) {
				...
				else if ((nxtElement.is('.treeview-menu'))
						&& (!nxtElement.is(':visible'))) {
					// Get the parent menu
				...
				if ((!curElement.is('.treeview-menu'))
						&& (curElement.is(':visible'))) {
					console.log("A");
					// Get the parent menu
					var parent = $this.parents('ul').first();
					// Close all open menus within the parent
					...
					// });
				} else if ((!curElement.is('.treeview-menu'))
						&& (!curElement.is(':visible'))) {
					console.log("B");
...
function addHistory(response, urlstring) {
	...
	if (pageurl != window.location) {
		...
		window.history.pushState({
			data : response
		}, '', win_loc + "#" + pageurlSplit[pageurlSplit.length - 1]
				+ (new Date).getTime());
	}
}
function getAjaxMenuHtml(divId, urlstring) {
	...
	$.ajax({
		...
		success : function(response) {
			...
			// $('#notification-bar').text('The page has been successfully
			// loaded');
			// DHTML
		},
		error : function() {
	...
	/*
	 * $.post( urlstring, function( response ) { $( '#'+divId ).html( response );
	 * });
	 */
}
...
function getAjaxJson(divId, urlstring) {
	$('#loader').modal('show');
	$('#search-term-form').attr("hidden", "");
	console.log("Div :" + divId + " url= " + urlstring);
	$.ajax({
...
function submitForm(divId, postUrl) {
	...
	/*
	 * $.post(postUrl,data,success = function(){ location.reload(); } );
	 */
	$('#' + divId).submit();
}
...
function submitFileWithJson(fobj, postUrl) {
...

	{
		...
		case "hidden":
			...
			// alert("Name :"+fobj.elements[i].name+":Value
		...
		case "file":
			...
			// alert("Element
			// :"+fobj.elements[i].name+"::"+escape(fobj.elements[i].files));
	...
	$
			.ajax({
				...
				processData : false,
				/*
				 * beforeSend: function(ajaxOptions){ var xhr = new
				 * XMLHttpRequest(); xhr.open("POST", postUrl,true);
				 * xhr.send(formData); console.log("XHR Status = "+xhr.status);
				...
				success : function(response) {
					...
					if (resp.type == "pass") {
						...
						if (resp.div == "div1") {
							...
							$('#upload').hide();
							$('#delete').show();
							$('#' + respDiv).css('background-image', 'url(' + resp.thumbUrl + ')');
							...
							label1: for (i = 1; i <= 5; i++) {
								...
								if ($('#url' + i).val() == "") {
									...
									$('#div' + i)
											.attr(
													'style',
													'width:100px; height:100px; background:no-repeat; background:url('
															+ resp.thumbUrl
															+ '); background-size:cover;background-position:center;border-radius: 2px;margin-left:55px;margin-top:5px;box-shadow:1px 1px 1px 1px rgba(0, 0, 0, 0.19), 1px 1px 1px 1px rgba(0, 0, 0, 0.19)');
									$('#div' + i).fadeOut("slow");
									$('#div' + i).fadeIn("slow");
									$('#url' + i).val(resp.url);
									...
									if (i == 6) {
										$('#serial_div' + i)
												.attr(
														...
														'width:100px; height:100px; background:no-repeat; background:url('
																+ resp.thumbUrl
																+ '); background-size:cover;background-position:center;border-radius: 2px;margin-left:45px;margin-top:25px;box-shadow:1px 1px 1px 1px rgba(0, 0, 0, 0.19), 1px 1px 1px 1px rgba(0, 0, 0, 0.19)');
									} else {
										$('#serial_div' + i)
												.attr(
														'style',
														'width:100px; height:100px; background:no-repeat; background:url('
																+ resp.thumbUrl
																+ '); background-size:cover;background-position:center;border-radius: 2px;margin-left:55px;margin-top:25px;box-shadow:1px 1px 1px 1px rgba(0, 0, 0, 0.19), 1px 1px 1px 1px rgba(0, 0, 0, 0.19)');
									}
...
function submitFormAllValues(fobj, postUrl) {
	...
	for (var i = 0; i < fobj.elements.length; i++) {
		...
		case "hidden":
			...
			break;
		/*
		 * case "button": str += fobj.elements[i].name + "=" +
		 * escape(fobj.elements[i].value) + "&";
		 * 
		 * if (fobj.elements[i].value === "Update") { //var btn = "'"+
		 * fobj.elements[i].name +"'";
		 * document.getElementById('btnedit').style.visibility = 'hidden'; } if
		 * (fobj.elements[i].value === "Save") { //var btn = "'"+
		 * fobj.elements[i].name +"'";
		 * document.getElementById('btnAdd').style.visibility = 'hidden'; }
		 * break;
		 */
		...
		success : function(response) {
			...
			if (resp.type == "html") {
				...
				if (resp.type == "pass") {
					...
					$('#respDiv2').addClass("alert alert-success");
					$('#iconDiv').addClass("fa fa-check-circle");
					...
					$('#respDiv3').addClass("alert alert-success");
					$('#iconDiv').addClass("fa fa-check-circle");
					...
					$('#respDiv').css({
						"visibility" : "visible",
						"color" : "#990000",
						"background-color" : "rgba(225, 177, 177, 0.8)"
					});
					$('#respDiv').addClass("alert alert-danger");
					$('#iconDiv').addClass("fa fa-warning");
					$('#respDiv').fadeIn(40).delay(2000).fadeOut(400);
					...
					$('#respDiv2').addClass("alert alert-danger");
					$('#iconDiv').addClass("fa fa-warning");
					...
					$('#respDiv3').addClass("alert alert-danger");
					$('#iconDiv').addClass("fa fa-warning");
					$('#respDiv3').fadeIn(40).delay(2000).fadeOut(400);
				}
...
function change(value) {
	if (value == 2) {
		console.log("ADV");
		document.getElementById("company_name").hidden = true;
		document.getElementById("business_name").hidden = false;
		document.getElementById("website").hidden = false;
	} else if (value == 3) {
		console.log("DBO");
		document.getElementById("company_name").hidden = false;
...
function getAjaxHtmlModal(divId, urlstring) {
    ...
    $.ajax({
        ...
        success: function(response) {
            console.log(response);
            $("#"+divId).html(response).modal({keyboard: true});
